...
Donde:
- \(\Lambda\) es una matriz diagonal con los autovalores (polos) del sistema en la diagonal.
- P es la matriz cuyas columnas son los autovectores correspondientes a cada autovalor.

Definiendo las nuevas variables de estado \( z = P^{-1} x \), la ecuación de estado se transforma en:

\[ \dot{z} = \Lambda z + P^{-1} B u \]

\[ y = C P z + D u \]

En esta forma, cada ecuación \(\dot{z}_i = \lambda_i z_i + (P^{-1} B)_i u\) está desacoplada de las demás, permitiendo un análisis modal directo. Los autovalores \(\lambda_i\) determinan la estabilidad y la dinámica de cada modo.
